...
import { getCarSeatAcademyLane, getCarSeatAcademyLanes } from '@/lib/guides/carSeatAcademy';
import type { GuideArticleRecord } from '@/lib/server/guideArticleRecord';

const CAR_SEAT_SUBGUIDE_HERO = {
  src: '/assets/editorial/gear.jpg',
  alt: 'Editorial car seat image for TMBC car seat category guides.',
} as const;
...
function buildLaneFitChecklistSections(lane: NonNullable<ReturnType<typeof getCarSeatAcademyLane>>) {
  return [
    {
      title: 'Strong fit if',
...
export default function CarSeatAcademyLanePage({
  guide,
  sourceRoute,
  displayDate,
  readingTime,
  ...
  const lane = getCarSeatAcademyLane(guide.slug);
  if (!lane) {
    return null;
  }

  const allLanes = getCarSeatAcademyLanes();
  ...
  const compareLinks = lane.compareAgainst
    .map((slug) => getCarSeatAcademyLane(slug))
  ...
  const whatPeopleGetWrong = dedupeTextItems(
    [
      'Choosing the feature or the promise first and only later checking whether the stage is actually right.',
      'Letting compatibility, premium marketing, or convenience language outrank install confidence and everyday use.',
      `Assuming ${lane.shortTitle.toLowerCase()} is automatically smarter because the category sounds more efficient on paper.`,
    ...
    [
      {
        ...
        label: 'Back to Car Seat Foundations',
        description: 'Return to the main academy car seat module if you want the wider category map again.',
        ...
        : []),
      {
        ...
        description: 'Useful once compatibility and click-in questions start affecting the car seat decision.',
        stage: 'Refine' as const,
      },
      {
        href: '/academy/registry/where-to-register',
        label: 'Bring This Into Registry',
        description: 'Use the category choice to keep the registry cleaner and a lot less random.',
        stage: 'Refine' as const,
      },
    ],
    4,
  ...
  const displayHeroImage = guide.heroImageUrl?.trim()
    ? {
        src: guide.heroImageUrl.trim(),
        alt: guide.heroImageAlt?.trim() || guide.title,
      }
    : lane.imageSrc
      ? { src: lane.imageSrc, alt: lane.imageAlt }
      : CAR_SEAT_SUBGUIDE_HERO;
        ...
        <div className="space-y-12">
            ...
            <div className="grid gap-8 lg:grid-cols-[minmax(0,1.2fr)_minmax(16rem,0.8fr)] lg:items-end">
              <div>
                <p className="text-[0.72rem] uppercase tracking-[0.28em] text-[#A15B72]">
                  TMBC Academy · Car Seat Category
                </p>
                ...
                <div className="mt-7 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
                  ...
                  <Link
                    href={compareLinks[0]?.href || '/academy/gear/car-seat-foundations'}
                  ...
                  >
                    {compareLinks[0] ? `Compare ${compareLinks[0].shortTitle}` : 'Back to Car Seat Foundations'}
                  </Link>
                </div>
              ...
              <div className="overflow-hidden rounded-[1.75rem] border border-white/70 bg-white/88 shadow-[0_18px_42px_rgba(58,36,43,0.10)]">
                <div className="relative aspect-[4/3] bg-[linear-gradient(135deg,rgba(251,245,239,0.98),rgba(247,231,236,0.92))]">
                  <Image
          ...
          <section className="rounded-[1.8rem] border border-[rgba(215,161,175,0.18)] bg-white/86 px-6 py-6 shadow-[0_18px_55px_rgba(58,36,43,0.08)] sm:px-8">
            <AcademyProgressBar
              ...
              label="Car seat category guide progress"
            />
          </section>
          ...
          <section className="space-y-8">
            <div className="max-w-3xl">
              ...
              <h2 className="mt-3 font-serif text-[2.2rem] leading-[0.96] tracking-[-0.04em] text-[#2F2430] sm:text-[2.6rem]">
                What actually defines this category
              </h2>
            </div>
          ...
          <section id="lane-fit" className="space-y-8">
            <ChecklistCardSet
              title="What this category means for you."
              description="Use this fit check before feature pages start doing too much of the talking."
              sections={buildLaneFitChecklistSections(lane)}
            />
          </section>

          <ComparisonTable
            title={`Compare ${lane.shortTitle} against the closest neighboring categories.`}
          ...
          <section className="space-y-6">
            <div className="max-w-3xl">
              ...
              <h2 className="mt-3 font-serif text-[2.1rem] leading-[0.97] tracking-[-0.04em] text-[#2F2430] sm:text-[2.4rem]">
                The mistakes that make a good category feel wrong
              </h2>
              <p className="mt-4 text-[1rem] leading-8 text-[#5B4B55]">
                These are usually the reasons parents talk themselves out of the right stage or let the wrong convenience story run the meeting.
              </p>
            </div>
          ...
          <ChecklistCardSet
            ...
            description="This is the part that keeps the category grounded. The useful answer shows up when the seat meets your actual car, your actual routine, and your actual patience."
          ...
          <section className="space-y-6">
            <div className="max-w-3xl">
              ...
              <p className="mt-4 text-[1rem] leading-8 text-[#5B4B55]">
                These examples make the category more concrete. They are here to sharpen the fit, not to turn the page into a trophy case with cup holders.
              </p>
            </div>
            ...
            <div className="grid gap-5 lg:grid-cols-2 xl:grid-cols-3">
              {lane.productExamples.map((example) => (
                <ProductPlaceholderCard
                  ...
                  standout={example.standout || example.typeLabel || lane.everydayFeel}
                  watchout={lane.tradeoff}
                  imageSrc={example.imageUrl}
                  imageAlt={example.imageAlt}
                  ctas={[
                    { label: '[PRODUCT_CARD_PLACEHOLDER]' },
                    { label: '[COMPARE_CTA_PLACEHOLDER: Compare This Car Seat]' },
          ...
          <section className="space-y-5">
            <ExpertTipCallout
              ...
              body="If you want help turning this category into a cleaner shortlist, travel-system plan, or registry decision, this is the right point to get tailored guidance."
            />
            {guide.consultationCtaEnabled !== false ? (
              <div className="rounded-[1.8rem] border border-[rgba(215,161,175,0.18)] bg-white/88 px-6 py-6 shadow-[0_18px_55px_rgba(58,36,43,0.08)] sm:px-7">
                ...
                <p className="mt-3 max-w-3xl text-base leading-8 text-[#5B4B55]">
                  Some car seat decisions need a calmer second opinion, especially when the category touches your vehicle fit, travel-system plan, or registry timing.
                </p>
                <div className="mt-5">
                  <GuideTrackedLink
                    guideId={guide.id}
          ...
          <SaveDecisionBar
            ...
            description="Use the category to save, compare, and validate the right options without slipping back into generic car seat browsing."
            actions={[
              { label: 'Back to Car Seat Foundations', href: '/academy/gear/car-seat-foundations' },
              compareLinks[0]
