  ...
  return (
    <MarketingSurface className="mx-auto max-w-6xl rounded-[1.75rem] border border-stone-200/70 bg-white/94 p-5 shadow-[0_16px_36px_rgba(0,0,0,0.04)] sm:p-6 md:rounded-[2rem] md:p-8 lg:p-10">
      <div className="max-w-3xl space-y-3">
        ...
        <p className="text-[0.98rem] leading-relaxed text-neutral-700">{startDescription}</p>
      </div>
      ...
      <div className="mt-7 grid items-start gap-6 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
        <div className="relative overflow-hidden rounded-[1.7rem] border border-[rgba(196,156,94,0.18)] bg-[linear-gradient(180deg,#fff8f8_0%,#f9f2ea_100%)] p-5 shadow-[0_18px_40px_rgba(0,0,0,0.04)] sm:p-6 md:p-7">
          ...
          <div className="relative space-y-6">
            <div className="space-y-3">
              ...
              <h3 className={`${questionTitleClassName} font-serif text-[2.15rem] leading-[0.98] tracking-[-0.04em] text-neutral-900 sm:text-[2.45rem]`}>
                {questionTitle}
              </h3>
            </div>

            {leadParagraph ? (
              <p className={`${leadParagraphClassName} text-[1rem] leading-8 text-neutral-700 sm:text-[1.04rem]`}>
                {leadParagraph}
              </p>
            ) : null}
          </div>
        ...
        <div className="space-y-4">
          {supportingParagraphs.map((paragraph, index) => (
            <div key={`${index}-${paragraph}`} className="rounded-[1.55rem] border border-stone-200/70 bg-[#fcfaf7] p-5 sm:p-6">
              ...
              <p className="mt-4 text-[1rem] leading-8 text-neutral-700">{paragraph}</p>
            </div>
          ))}
          ...
          {callout ? (
            <div className="rounded-[1.55rem] border border-[rgba(232,154,174,0.26)] bg-[linear-gradient(180deg,#fffdfd_0%,#f9edf1_100%)] p-5 shadow-[0_14px_36px_rgba(0,0,0,0.05)] sm:p-6">
              ...
              <p className="mt-4 text-[1rem] leading-8 text-[var(--color-accent-dark)]/92">{callout.body}</p>
            </div>
          ) : null}
        </div>
      </div>
