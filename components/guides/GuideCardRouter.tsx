...
export default function GuideCardRouter({
  ...
  return (
    <section className="space-y-5">
      ...
      <div className="-mx-4 flex snap-x snap-mandatory gap-3.5 overflow-x-auto px-4 pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden md:-mx-0 md:grid md:grid-cols-2 md:gap-4 md:overflow-visible md:px-0 xl:grid-cols-3">
        {cards.map((card) => (
          <Link
            key={`${card.href}-${card.title}`}
            href={card.href}
            className="group flex min-w-[86vw] max-w-[19rem] snap-start flex-col rounded-[1.35rem] border border-[rgba(215,161,175,0.16)] bg-[rgba(252,247,249,0.92)] p-4 shadow-[0_14px_34px_rgba(0,0,0,0.05)] transition duration-300 hover:-translate-y-1 hover:bg-white hover:shadow-[0_20px_50px_rgba(58,36,43,0.10)] md:min-w-0 md:max-w-none md:rounded-[1.5rem] md:p-5"
          >
