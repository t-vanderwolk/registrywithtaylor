...
export default function GuideComparisonCards({
  ...
  return (
    <section className="min-w-0 rounded-[2rem] border border-[rgba(196,156,94,0.18)] bg-[linear-gradient(180deg,#fff9f5_0%,#fbf4ec_100%)] p-6 shadow-[0_18px_50px_rgba(0,0,0,0.05)] md:p-8">
      ...
      <div className="mt-6 grid min-w-0 grid-cols-1 gap-6 md:grid-cols-2 2xl:grid-cols-3">
        {cards.map((card) => (
          <Link
            key={`${card.href}-${card.title}`}
            href={card.href}
            className="group flex min-w-0 h-full flex-col rounded-[1.4rem] border border-black/6 bg-white/92 p-5 shadow-[0_12px_30px_rgba(0,0,0,0.04)] transition duration-200 hover:-translate-y-1 hover:border-[rgba(196,156,94,0.24)] hover:shadow-lg"
          >
