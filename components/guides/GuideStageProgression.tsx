...
export default function GuideStageProgression({
  ...
  return (
    <section
      id={id}
      className="rounded-[1.75rem] border border-[rgba(196,156,94,0.18)] bg-[linear-gradient(180deg,#fffaf6_0%,#fbf4ec_100%)] p-5 shadow-[0_18px_44px_rgba(0,0,0,0.05)] sm:p-6 md:rounded-[2rem] md:p-8"
    >
      <div className="space-y-2">
        ...
        {description ? <p className="max-w-[72ch] text-[0.96rem] leading-relaxed text-neutral-700">{description}</p> : null}
      </div>

      <div className="mt-6 grid gap-4 xl:grid-cols-4">
        {items.map((item, index) => {
          const card = (
            <div className="group relative flex h-full flex-col rounded-[1.45rem] border border-black/6 bg-white/90 p-4 shadow-[0_12px_28px_rgba(0,0,0,0.04)] transition duration-200 hover:-translate-y-0.5 hover:border-[rgba(196,156,94,0.24)]">
              {index > 0 ? (
              ...
              <h3 className="mt-4 font-serif text-[1.28rem] leading-[1.08] tracking-[-0.03em] text-neutral-900">{item.title}</h3>
