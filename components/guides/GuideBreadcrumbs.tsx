...
export default function GuideBreadcrumbs({
  ...
  return (
    <RevealOnScroll>
      ...
      >
        <ol className="flex min-w-0 flex-wrap items-center gap-2 text-[0.68rem] uppercase tracking-[0.18em] text-[#8F4C62] sm:text-[0.72rem] sm:tracking-[0.24em]">
          {items.map((item, index) => (
            <li key={`${item.label}-${index}`} className="inline-flex min-w-0 flex-wrap items-center gap-2">
              ...
              {item.href ? (
                <Link href={item.href} className="max-w-full break-words transition duration-200 hover:text-[#6E3148]">
                  {item.label}
                </Link>
              ) : (
                <span className="max-w-full break-words text-[#4B3641]">{item.label}</span>
              )}
            </li>
          ))}
        </ol>
