...
import {
  ...
  getDailyUseGearCategory,
  getDailyUseGearNextStepLinks,
  type DailyUseGearCategorySlug,
} from '@/lib/guides/dailyUseGearLane';

const CATEGORY_SLIDES = [
  { id: 'orientation', label: 'Orientation', shortLabel: 'Start' },
  { id: 'what-it-does', label: 'What This Product Does', shortLabel: 'Job' },
...
export default function DailyUseGearCategoryPage({
  slug,
}: {
  ...
  const nextSteps = getDailyUseGearNextStepLinks(slug);

  return (
    <GuideSlideDeck
      containerId={`daily-use-gear-${slug}-carousel`}
      items={CATEGORY_SLIDES.map((item) => ({
        ...item,
        id: `daily-use-gear-${slug}-${item.id}`,
      }))}
      backLink={{ href: DAILY_USE_GEAR_HUB_PATH, label: 'Back to Daily Use Gear' }}
      ecosystemCurrentStep={6}
      journeyPathLabels={[...DAILY_USE_GEAR_JOURNEY_PATH, category.title]}
    >
      <SlideSection
        id={`daily-use-gear-${slug}-${CATEGORY_SLIDES[0].id}`}
        background="ivory"
                ...
                <div className="grid gap-0 lg:grid-cols-[minmax(0,1fr)_minmax(22rem,0.9fr)]">
                  <div className="space-y-6 px-6 py-8 md:px-8 md:py-10 lg:px-10 lg:py-12">
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap">
                      <a
                        href={`${path}#daily-use-gear-${slug}-${CATEGORY_SLIDES[1].id}`}
                      ...
                      <a
                        href={`${path}#daily-use-gear-${slug}-${CATEGORY_SLIDES[5].id}`}
                    ...
                    <GuideHandwrittenNote
                      title="This category should make the day easier in motion."
                      description={
                        <p>
                          If the product only sounds impressive in theory, it is probably not the daily-use winner. The point is ease, not
                          performance art.
                        </p>
                      }
                      tone="linen"
                  ...
                  <div className="border-t border-[rgba(215,161,175,0.14)] bg-[linear-gradient(180deg,#fdf6f7_0%,#fffdfc_100%)] lg:border-l lg:border-t-0">
                    ...
                    <div className="space-y-3 px-6 py-6 lg:px-8 lg:py-8">
                      <div className="rounded-[1.3rem] border border-[rgba(215,161,175,0.16)] bg-white/88 px-4 py-4">
                        <p className="text-[0.68rem] uppercase tracking-[0.22em] text-[#A15B72]">Lane</p>
                        <p className="mt-2 text-base font-medium leading-7 text-[#2F2430]">Daily Use Gear</p>
                      </div>
                      <div className="rounded-[1.3rem] border border-[rgba(215,161,175,0.16)] bg-white/88 px-4 py-4">
                      ...
                      <div className="rounded-[1.3rem] border border-[rgba(215,161,175,0.16)] bg-white/88 px-4 py-4">
                        ...
                        <p className="mt-2 text-base font-medium leading-7 text-[#2F2430]">{nextSteps[0]?.label ?? 'Book a Consultation'}</p>
                      </div>
                    </div>
                  </div>
                </div>
      ...
      <SlideSection id={`daily-use-gear-${slug}-${CATEGORY_SLIDES[1].id}`} background="white">
        <GuideJourneyIntro
      ...
      <SlideSection id={`daily-use-gear-${slug}-${CATEGORY_SLIDES[2].id}`} background="blush">
        <GuideBulletSection
          eyebrow="Types"
          title="Types"
          description={category.typesDescription}
          items={category.types}
        />
      </SlideSection>

      <SlideSection id={`daily-use-gear-${slug}-${CATEGORY_SLIDES[3].id}`} background="ivory">
        <GuideBulletSection
      ...
      <SlideSection id={`daily-use-gear-${slug}-${CATEGORY_SLIDES[4].id}`} background="white">
        <GuideBulletSection
      ...
      <SlideSection id={`daily-use-gear-${slug}-${CATEGORY_SLIDES[5].id}`} background="blush">
        <DecisionBlock
          title="How to Choose"
      ...
      <SlideSection id={`daily-use-gear-${slug}-${CATEGORY_SLIDES[6].id}`} background="ivory">
        <NextSteps
          title="Next Step"
          description="Keep the lane moving while the logic is still clear. The goal is not to read everything. The goal is to leave with the next useful move."
          links={nextSteps}
        />
      </SlideSection>
    </GuideSlideDeck>
