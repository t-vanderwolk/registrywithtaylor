...
export default function HomeAuthorityStrip({
  ...
  /** Optional standalone badge/CTA shown to the right of the credential strip. */
  badge?: ReactNode;
}) {
  return (
      ...
      <div className="mx-auto max-w-6xl px-6">
        <RevealOnScroll>
          <div
            className={
              badge
                ? 'flex flex-col items-center gap-6 md:flex-row md:items-center md:justify-between md:gap-10'
                : 'flex flex-col items-center gap-4 text-center'
            }
          >
            <div className={badge ? 'flex flex-col items-center gap-4 text-center md:items-start md:text-left' : 'contents'}>
              <p className="max-w-4xl text-[0.78rem] uppercase tracking-[0.16em] text-black/52 md:text-[0.82rem]">
                {text}
              </p>

              {logos.length > 0 ? (
                <div className={`flex flex-wrap items-center gap-x-7 gap-y-3 md:gap-x-10 ${badge ? 'justify-center md:justify-start' : 'justify-center'}`}>
                  {logos.map((logo) => (
