...
const ACCESSORY_HINT =
  /\b(adapter|adapters|cup\s*holder|cupholder|tray|bassinet\s+(stand|fabric|bundle|adapter)|bumper|backpack|rain\s*cover|footmuff|liner|board\b|organi[sz]er|second\s*seat|sibling\s*seat|snack|bottle\s*cage|carry\s*bag|travel\s*bag|travel\s*system|canopy|sun\s*shade|parasol|walker)\b/i;
